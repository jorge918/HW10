...
class CarModel():
    ...
    def __init__(self):
        ...
        self.mink1 = self.m1 * 9.81 * 0.075    #If I jack up my car and release the load on the spring, it extends about 3 inches
        self.maxk1 = self.k1 / 5.0  #What would be a good value for a soft spring vs. a stiff spring?
        self.mink2 = self.k2 / 10.0  #Same question for the shock absorber.
        self.maxk2 = self.k2 * 2.0
        self.accel =None
        self.accelMax = None
        self.accelLim = 5.0
        self.SSE = 0.0
...
class CarView():
    ...
    def buildScene(self):
        ...
        self.gv_#Schematic.setScene(self.scene)
...
class CarController():
    ...
    def ode_system(self, X, t):
        ...
        x1ddot = x1ddot = (1 / self.model.m1) * (-self.model.c1 * (X[1] - X[3]) - self.model.k1 * (X[0] - X[2]))
        x2ddot = x2ddot = (1 / self.model.m2) * (-self.model.c1 * (X[3] - X[1]) - self.model.k2 * (X[2] - X[0]))
    ...
    def calculate(self, doCalc=True):
        ...
        self.model.m1 = float(self.le_m1.text()) # Mass of car body in kg
        self.model.m2 = float(self.le_m2.text())  # Mass of wheel in kg
        self.model.c1 = float(self.le_c1.text()) # Damping coefficient in N*s/m
        self.model.k1 = float(self.le_k1.text()) # Spring constant of suspension in N/m
        self.model.k2 = float(self.le_k2.text())  # Spring constant of tire in N/m
        self.model.v = float(self.le_v.text())  # Velocity of car in kph

        #recalculate min and max k values
        self.mink1=288966.54 # Minimum value for suspension spring constant
        self.maxk1= 57933.07  # Maximum value for suspension spring constant
        self.mink2=121015.75  # Minimum value for tire spring constant
        self.maxk2= 242031.50  # Maximum value for tire spring constant
    ...
    def OptimizeSuspension(self):
        ...
        #Step 3:
        answer= minimize(self.SSE, x0, method='nelder-mead', options={'disp': True})
        optimized_params = result.x  # Optimized values of [k1, c1, k2]
        self.model.k1, self.model.c1, self.model.k2 = optimized_params
        self.view.updateView(self.model)
